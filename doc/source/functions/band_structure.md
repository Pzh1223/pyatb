...
`solver`: Specifies the algorithm used to diagonalize the Hamiltonian matrix. The default is `dense`, which calculates all eigenvalues using LAPACK's dense matrix routines. Setting `solver` to `parpack` (or `sparse`) uses the generalized Shift-Invert mode (mode=3) of the PARPACK Lanczos solver, which computes only the eigenvalues near the Fermi level (`fermi_energy`), significantly speeding up calculations for large systems (e.g., tens of thousands of atoms). This requires specifying `band_range` to define how many eigenvalues near the Fermi level to calculate. Note that if the number of requested eigenvalues is equal to or greater than the basis size minus one (`cal_band_num >= basis_num - 1`), the solver will automatically fall back to the dense `eigh` routine because PARPACK requires computing fewer eigenvalues than the matrix dimension.
